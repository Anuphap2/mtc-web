    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input id="image" class="block mt-1 w-full border p-2 rounded" type="file" name="image" accept="image/*"> {{-- Added accept attribute --}}
            ...
            <div class="mb-4">
                ...
                <input id="pdf" class="block mt-1 w-full border p-2 rounded" type="file" name="pdf" accept=".pdf"> {{-- Added accept attribute --}}
                <x-input-error :messages="$errors->get('pdf')" class="mt-2" />

                @if($post->pdf_path)
                    <div class="mt-4">
                        <p class="text-sm font-medium text-gray-700">ไฟล์ PDF ปัจจุบัน:
                            <a href="{{ Storage::url($post->pdf_path) }}" target="_blank" class="text-blue-500 hover:text-blue-700 ml-2">
                                [ ดูไฟล์ PDF ]
                            </a>
                        </p>
                        {{-- เพิ่ม Checkbox สำหรับลบ PDF --}}
                        <label for="remove_pdf" class="inline-flex items-center mt-2">
                            <input type="checkbox" id="remove_pdf" name="remove_pdf" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                            <span class="ms-2 text-sm text-gray-600">{{ __('ลบไฟล์ PDF นี้') }}</span>
                        </label>
                    </div>
                @endif
            </div>

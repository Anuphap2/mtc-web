...
<a href="{{ route('post.show', $post) }}"
    ...
    <div class="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-black/70 to-transparent text-white">
        <h3 class="font-semibold text-lg line-clamp-2">{{ $post->title }}</h3>
        <p class="text-xs text-gray-300 mt-1">{{ $post->created_at?->addYears(543)->format('d/m/Y') }}</p>
    </div>
</a>
